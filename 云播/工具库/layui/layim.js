...
import laytpl from '@/assets/layui/laytpl.js'
import layer from '@/assets/layui/layer.js'
import xMessager from '@/components/xMessager/chat/xMessager'
import faceslib from '@/utils/face'
...
//基础配置
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++;  if(data.more ==1){ }}'
      ...
      ,'{{# } else{ }}'
      ,'<li class="userlist {{# if(data.class_name){}}{{data.class_name}}{{# } }}" {{# if(data.groupid == 4){}} layim-event="iframe_v" {{# }else{ }} layim-event="chat" {{# } }} data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" id="layim-'+ options.type +'{{ data.id }}"><img src="{{# var url = data.avatar; }}{{url}}"><div class="name">{{data.username||data.groupname||data.name||"佚名"}}{{# if(data.me_level>=500){var datauid="("+data.id.split("-")[1]+")"}}{{datauid}}{{# }}} </div><p class="act">{{# if(data.title >= 0){}}<a class="identity_icon {{# if(data.level<500){}} identity_icon{{data.title}}{{# } }}" href="javascript:">{{# if(data.level==500){ }}<i class="teacher_icon"></i>{{#}else if(data.level==900){}}<i class="manager_icon"></i>{{#}else if(data.level ==2000){}}<i class="owner_icon"></i>{{#}else if(data.level ==4000){}}<i class="teamer_icon"></i>{{#}else{}}<i class="rank{{data.title}}"></i>{{#}}}</a>{{# } }}{{# if(data.ip  && data.me_level > 500){ }}<s class="icon_z" title="真实用户"></s>{{# } }}{{# if(data.ch  && data.me_level > 500){ }}<s class="em1" title="推广用户"></s>{{# } }} {{# if(data.is_mobile ==1){ }}<s class="h5_icon" title="手机用户"></s>{{# } }} {{# if(data.is_mobile ==2){ }}<s class="android_icon" title="安卓用户"></s>{{# } }} {{# if(data.is_mobile ==3){ }}<s class="apple_icon" title="苹果用户"></s>{{# } }} {{# if(data.ip  && ((data.me_level> 500 && data.level<500) || data.me_level==6000)){ }}<span class="ip_zone">{{data.zone}}</span>{{# } }}</p></li>'
    ,'{{# } }); if(length === 0){ }}'
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'</ul>'
    ,'<div class="main_content"><ul class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ,'{{# if(d.mine.level >=900){ }}'
      ,'<li class="layui-icon groupbtn"  layim-event="groupsend" title="群发">&#xe63a;</li>'
      ,'{{# } }}'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}} layui-show">'
    ,'<div class="layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ,'<img src="{{# var url = d.data.avatar; }}{{url}}"><div class="des"><span layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name || d.data.username ||"佚名" }} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}  {{# if( d.data.me_level>500){ var datauid = "("+d.data.id.split("-")[1]+")"; }} {{ datauid }}  {{# } }}  </span>'

    ,'{{# if(d.data.ip && ((d.base.mine.level > 500 && d.data.level<500) || d.base.mine.level ==6000 )){ }}'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span title="送礼物" layim-event="gift" class="gift"><i class="i1"></i></span>'
        ...
        ,'<div class="layim-tool-all" layim-event="alltool"><p><i></i>管理功能</p><div class="down"><em></em><div class="dinner"><dl>'
        ,'{{# }else { }}'
        ...
        //,'{{# }; }}'
        ,'<dd><a href="javascript:;" layim-event="m_disable_Accout" class="m_disable_ip"><span></span>封账号</a></dd>'
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    post(options.init, function(data){
      ...
      if(local.close && options.inset == 0){
        popmin();
      }
  ...
  var setChatMin = function(newMsg){
    var thatChat = newMsg || thisChat().data, base = layim.cache().base;
    if(layimChat && !newMsg){
      layimChat.hide();
    }
    ...
    layer.open({
      ...
      ,offset: $(window).height() - $('.chatInput').height()-40
      ...
      ,success: function(layero, index){
        ...
        $(window).resize(function(){
            var newtop = $(window).height()-$('.chatInput').height()-40;
  ...
  var sendMessage = function(){
    if(window.sendTime != 0){
       layer.msg('发送消息太快了，请稍等'+(4-window.sendTime)+'秒', {icon: 5});
       return
    }
    var data = {
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      ...
      pushChatlog(message);
      var param2 = {};
      param2.room_id = xMessager.room_id;
      param2.frid = param.to.room_id;
      param2.to_uid = param.to.uid;
      param2.secret = 1;
      param2.message = param.mine.content;
      xMessager.message(param2)
          ...
          layui.each(cache.friend, function(index1, item1){
            layui.each(item1.list, function(index, item){
              if(item.id == data.id){
                ...
                item.risknewmsg = item.risknewmsg;
    ...
    if(thisChatlog){
      ...
      layui.each(thisChatlog, function(index, item){
        ...
        && item.content === message.content)){
          nosame = true;
          if(item.risknewmsg != message.risknewmsg){
             thisChatlog.splice(thisChatlog.indexOf(item),1,message)
          }
        }
      });
  ...
  //系统消息 风险提示
  var riskSystem = function(){
    // if(userpara.levelinroom>=500){
    //   return false;
    // }
    // var item = {
    //   avatar:cdn+'/resources/images/avatar.png',
    //   content:'直播间讲师观点不代表本站立场，投资有风险，入市须谨慎！',
    //   id:'member-000000',
    //   username:'系统消息'
    // };
    // var thatChat = thisChat();
  ...
  var viewChatlog = function(){
    ...
    ,riskIndex = 0;
    if(typeof(chatlog[thatChat.data.type + thatChat.data.id]) == 'undefined'){
       riskSystem();
    }else{
      layui.each(chatlog[thatChat.data.type + thatChat.data.id], function(index, item){
        if(item.risknewmsg == true && riskIndex == 0){
            riskSystem();
            riskIndex = 1;
        }
        ul.append(laytpl(elemChatMain).render(item));
        item.risknewmsg =false;
        pushChatlog(item);
      });
      if(riskIndex==0){
         riskSystem();
  ...
  var faces = function(){
    ...
    layui.each(alt, function(index, item){
       //arr[item] =  '@/components/Face/img/'+ index + '.png';
       arr[item] = faceslib[index].img;
    });
    return arr;
  }();
  ...
  var anim = 'layui-anim-upbit', events = {
    //切换工具显示
    alltool:function(othis, e){
      othis.toggleClass('layim-tool-all-show')
    },
    //在线状态
    ...
    //关于
    ,about: function(){
      layer.alert('版本： '+ v + '<br>版权所有：<a href="#" target="_blank">layim.layui.com</a>', {
        title: '关于 LayIM'
        ,shade: false
      });
    }
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        type: 2
        ,maxmin: false
        ,title: '与 '+ thatChat.data.username +' 的聊天记录'
        ,area: ['450px', '100%']
        ,shade: false
        ,offset: 'rb'
        ,skin: 'layui-box'
        ...
        ,content:  cache.base.chatLog + '?to_uid=' + thatChat.data.id.split('-')[1] + '&type=room'+ '&room_id='+thatChat.data.room_id
      });
    }
    ...
    ,gift: function(othis, e){
      ...
      var that_uid = thatChat.data.id.split('-')[1];
       $('.giftbox').show().css('zIndex',1000000000);
       $('.giftbox h4 .name').text(thatChat.data.name);
       $('.giftbox').attr('data-uid',that_uid);
       $('.giftbox li').removeClass('active').first().addClass('active');
       $('.giftbox .act span').removeClass('active').first().addClass('active');
    }
    ,set_manage: function(othis, e){
      var thatChat = thisChat();
      g_UserList.menu_selected_uid = thatChat.data.id.split('-')[1];
      xMessager.UserAction();
    ...
    ,m_disable_chat: function(othis, e){
      var thatChat = thisChat();
      var uid = thatChat.data.id.split('-')[1];
      xMessager.UserAction({'query_uid':uid,'type':1});
      events.tool_hide(othis);
    }
    ,m_disable_ip: function(othis, e){
      var thatChat = thisChat();
      var uid = thatChat.data.id.split('-')[1];
      xMessager.UserAction({'query_uid':uid,'type':4});
      events.tool_hide(othis);
    }
    ,m_disable_Accout: function(othis, e){
      var thatChat = thisChat();
      var uid = thatChat.data.id.split('-')[1];
      xMessager.UserAction({'query_uid':uid,'type':3});
      events.tool_hide(othis);
    }
    ,m_kick: function(othis, e){
      var thatChat = thisChat();
      var uid = thatChat.data.id.split('-')[1];
      xMessager.UserAction({'query_uid':uid,'type':2});
      events.tool_hide(othis);
    }
    ,tool_hide:function(){
