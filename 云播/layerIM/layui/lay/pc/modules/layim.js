...
layui.define(['jquery', 'layer', 'laytpl', 'upload'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li class="userlist" layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" id="layim-'+ options.type +'{{ data.id }}"><img src="{{ data.avatar }}"><div class="name"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span>    {{# if(data.me_level>500){ var datauid = "("+data.id.split("-")[1]+")" }} {{ datauid }}  {{# } }} </div><p class="act">{{# if(data.level==2000){ }}<i class="i1" title="房主"></i>{{# } else if(data.level==900) { }}<i class="i2" title="管理"></i>{{# } else if(data.level==500) { }}<i class="i3" title="老师"></i>{{# } else if(data.level==100) { }}<i class="i4" title="会员"></i>{{# } else if(data.level<=20) { }}<i class="i5" title="游客"></i>{{# } }} {{# if(data.title>0){}}<b class="userRand userRand{{data.title }}">LV{{data.title }}</b> {{# } }} {{# if(data.inout_log_id  && data.me_level >= roompara.user_info_level){ }}<em class="em2" title="真实用户"></em>{{# } }}{{# if(data.ch  && data.me_level >= roompara.user_info_level){ }}<em class="em1" title="推广用户"></em>{{# } }}{{# if(data.is_mobile){ }}<em class="em3" title="手机用户"></em>{{# } }}{{# if(data.ip  && ((data.me_level>= roompara.user_info_level && data.level<500) || data.me_level==6000)){ }}{{data.zone}}{{# } }}</p></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'</ul>'
    ,'<ul class="layim-tab-content layim-show layim-list-friend">'
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span class="groupName">{{ item.groupname||"未命名分组"+index }}</span><em>{{# if(index > 0){ }}(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>){{# }else { }}<cite id="im_user_num">(0)</cite>{{# } }}</em></h5>'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ,'<div class="layim-chat-title">'
      ,'<a class="layim-chat-other">'
        ,'<img src="{{ d.data.avatar }}"></a><div class="des"><span layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"佚名" }} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}  {{# if(d.data.me_level>500){ var datauid = "("+d.data.id.split("-")[1]+")" }} {{ datauid }}  {{# } }}  </span>'
  ...
  layui.laytpl.content = function(content){
    ...
    .replace(/\[([^\s\[\]]+?)\]/g, function(face){  //转义表情
      ...
      return '<img width="30" height="30" alt="'+ alt +'" title="'+ alt +'" src="' + faces[alt] + '">';
  ...
  var layimMain, popim = function(content){
    ...
    return layer.open({
      ...
      ,success: function(layero){
        ...
        //     var target = $(e.target);
        //     if(target.closest(".layui-layer-title, .layui-layim-min, .layui-layer-content").length == 0){
        //       if($('.layui-layim').is(':visible')){
        //          popmin();
        //       }
        //     }
        // });
  ...
  var sendMessage = function(){
    var data = {
      ...
      ,level: cache.mine ? cache.mine.level : null
      ,mine: true
    };
  ...
  var getMessage = function(data){
    data = data || {};
    //data.content = roomFuc.faceContent(data.content);
    ...
    $('.layui-layer').find('.msgTip').addClass('newmsgTip');

    // if($('.layui-layer').hasClass('layminmsg')){
    //    $('.layui-layer').find('.msgTip').addClass('newmsgTip');
    //    //消息题型定时器
    //     if($(layimMin).find('.newmsgTip') && !msgTimer){
    //        msgTimer = setInterval(function(){
    //         if($('.newmsgTip').length){
    //           $('.newmsgTip').addClass('newmsgSpanshow');
    //           $('.newmsgTip').toggleClass('on');
     ...
     events.face.index = layer.tips(content, othis, {
        ...
        ,success: function(layero){
          layero.find('.layim-face-list>li').on('mousedown', function(e){
            ...
            // focusInsert(thatChat.textarea[0], 'face22' +  this.title + ' ');
