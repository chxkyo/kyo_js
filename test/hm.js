(function() {
        ...
        var q = void 0     // undefined
            , r = !0        // true
            , t = null     // null
            , u = !1;       // false

        // TODO: mt对象封装各种工具方法
        // TODO: mt.cookie 对象 封装相关cookie操作
        mt.cookie = {};
        ...
        // 用来定位一个节点，
        // 返回的结果
        // 1. #small_content>div[1]
        // 2. html>body>div>div>div>div[1]
        // b boolean 类型 true: 找到一个有id的祖先元素就break,f[0]      false：同时还要找到html,放到f[1], 如果一直没有id，则放到f[0]
        // push 到f， 所有 id的先进，如果有的话
        mt.h.J = function(a, b) {
            var d = []
                , f = [];
            if (!a)
                return f;
            // a.parentNode 父节点 不为 null
            for (; a.parentNode != t; ) {
                // 父节点的子节点个数（兄弟节点数+1）
                // n 相同nodeName节点计数，索引 [n]
                for (var g = 0, n = 0, l = a.parentNode.childNodes.length, p = 0; p < l; p++) {
                    var e = a.parentNode.childNodes[p];
                    if (e.nodeName === a.nodeName && (g++,
                        e === a && (n = g),
                        0 < n && 1 < g))
                        break
                }
                ...
                if ((l = "" !== a.id) && b) {
                    ...
                    l && (l = "#" + encodeURIComponent(a.id),
                        ...
                        f.push(l)),
                        d.unshift(encodeURIComponent(String(a.nodeName).toLowerCase()) + (1 < g ? "[" + n + "]" : ""));
                // 类似递归调用..
                a = a.parentNode
            }
            f.push(d.join(">"));
        ...
        // TODO: mt.localStorage 对象, 封装本地存储相关操作
        mt.localStorage = {};
        ...
        // TODO: mt.sessionStorage 对象, 封装会话存储相关操作
        mt.sessionStorage = {};
        ...
        // TODO: 发送请求, 这个文件共有三处调用
        mt.aa = {};
        // 打开一次页面到加载完成发送四次请求, window 对象有四个 mini_tangram_log_xxxxxx 属性。。
        // b 为回调，接受一个a参数
        // TODO: 发送请求
        mt.aa.log = function(a, b) {
            ...
            ;
            d.src = a + "&ref=" + encodeURIComponent(document.referrer)
        }
        ;

        // window["statistic_log"] = mt.aa.log;
        window["statistic_log"] = function(p) {
            if (! p) return;
            mt.aa.log("http://" + h.D.$ + "?url=" + encodeURIComponent(window.location.href) + "&" + p);
        };

        // TODO: mt.S 对象  检测 flash 支持
        mt.S = {};
        mt.S.ua = function() {
            var a = "";
            // navigator.plugins 为 PluginArray, 空数组boolean值为真
            if (navigator.plugins && navigator.mimeTypes.length) {
                var b = navigator.plugins["Shockwave Flash"];
                b && b.description && (a = b.description.replace(/^.*\s+(\S+)\s+\S+$/, "$1"))       // 版本号
            } else if (window.ActiveXObject)    // 浏览器兼容
                try {
                    if (b = new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))
                        (a = b.GetVariable("$version")) && (a = a.replace(/^.*\s+(\d+),(\d+).*$/, "$1.$2"))     // 版本号
                } catch (d) {}
            return a
        }
        ;
        ...
        // TODO: mt.url 对象
        mt.url = {};
        // 从url a 的 get 参数和 锚点 中, 获取 b 的值
        mt.url.i = function(a, b) {
            var d = a.match(RegExp("(^|&|\\?|#)(" + b + ")=([^&#]*)(&|$|#)", ""));
            return d ? d[3] : t
        }
        ;
        // 返回a的协议类型 http: 或 https: （包含冒号）
        mt.url.ab = function(a) {
            return (a = a.match(/^(https?:)\/\//)) ? a[1] : t
        }
        ;
        // 返回a的根地址, 包含主机名、域名、端口 或 IP地址、端口
        ...
        (function() {
                function a() {
                    for (var a = u, d = document.getElementsByTagName("script"), f = d.length, f = 100 < f ? 100 : f, g = 0; g < f; g++) {
                        var n = d[g].src;
                        if (n && 0 === n.indexOf("http://sta.1234tv.com/h")) {
                            a = r;
                            break
                        }
                    }
                ...
                // h 也是全局对象, h.na 方法判断统计js是否已经包含在页面中
                return h.na = a //  函数 a
            }
        )();
        var A = h.na;
        h.D = {
            bb: "http://sta.1234tv.com/ico",
            $: "sta.1234tv.com",
            ea: "tj.com",
            ya: "hmmd",
            za: "hmpl",
            ...
            O: A() || "https:" === document.location.protocol ? "https:" : "http:",     // A() 已包含统计js 返回 true 否则返回null，必须是src="http://..." 开头的统计js
            eb: 0,
            Ta: 6E5,
            gb: 5E3,
            Ua: 5,
            ...
            Sa: 1,
            P: 2147483647,
            // ba: "cc cf ci ck cl cm cp cu cw ds vl ep et fl ja ln lo lt rnd si su v cv lv api sn ct u tt url".split(" ")
            ba: "ep fl lt cv api sn url".split(" ")
        };
        (function() {
                var a = {
                    p: {},
        ...
        (function() {
                ...
                var g = mt.event, n = h.G, l = r, p = r, e = r, k = r, w = +new Date, m = w, s = 0, y = r, v = d("visibilityState"), x = d("hidden"), z;
                b();
                h.F = {
                    sa: function() {
                        return +new Date - w
...
/*                        e.e = 0;
                    ...
                    ia: function() {
                        ...
                        if (!a || d.d(a, "Array"))
                            window._ybt = {
                                ...
                                cmd: {},
                                push: function() {
                                    for (var a = window._ybt, k = 0; k < arguments.length; k++) {
                                        var b = arguments[k];
                                ...
                                window._ybt.cmd[c.id] = [],
                                window._ybt.push.apply(window._ybt, a)
                    },
                    la: function() {
                        var a = window._ybt;
        ...
        (function() {
                function a() {
                    "undefined" === typeof window["_bdhm_loaded_" + c.id] && (window["_bdhm_loaded_" + c.id] = r,
                        this.a = {},
                        this.X = r,
                        this.M = u,
                        this.init())
                ...
                a.prototype = {
                    ...
                    Ka: function() {
                        var a, b, d, e, f;
                        // k.r 最近一次访问这个站点的时间
                        k.r = this.getData("Yb_lpvt_" + c.id) || 0;
                        // 保留到秒
                        13 === k.r.length && (k.r = Math.round(k.r / 1E3));
                        b = this.ta();
                        a = 4 !== b ? 1 : 0;
                        // Yb_lvt_id 第一次访问的时间
                        // Yb_lvt_id 保存在 cookie localStorage, 长期保存
                        // Yb_lpvt_id 保存在 cookie sessionStorage, 会话保存
                        // getData 方法cookie sessionStorage localStorage顺序取值
                        if (d = this.getData("Yb_lvt_" + c.id)) {
                            e = d.split(",");
                            ...
                            d = k.m,
                                e = "",
                                f = 1;
                        // 每次都set，当已存在 lvt 时，其值不会改变
                        // lpvt 每次都会改变
                        this.setData("Yb_lvt_" + c.id, d, c.age);
                        this.setData("Yb_lpvt_" + c.id, k.m);
                        d = k.m === this.getData("Yb_lpvt_" + c.id) ? "1" : "0";
                        ...
                        this.a.lt = e;
                        this.a.lv = f;
                        this.a.url = window.location.href;
                    },
                    Ja: function() {
                    ...
                    init: function() {
                        try {
                            ...
                            d.log(k.O + "//" + k.$ + "?" + b.join("&"))                     // TODO: anchor 3rd ? 异常执行。。
                        }
                    },
                    Ha: function() {
                        function a() {
                    ...
                    k: function(a) {
                        ...
                        w.w("stag-d");
                        b.da(e);
                        d.log(e, function(d) {              // TODO: anchor 2nd
                            // b.R(d);
                    ...
                    da: function(a) {
                        var b = e.get("Yb_unsent_" + c.id) || ""
                            , d = this.a.u ? "" : "&u=" + encodeURIComponent(document.location.href)
                            , b = encodeURIComponent(a.replace(/^https?:\/\//, "") + d) + (b ? "," + b : "");
                        e.set("Yb_unsent_" + c.id, b)
                    },
                    R: function(a) {
                        var b = e.get("Yb_unsent_" + c.id) || "";
                        b && (a = encodeURIComponent(a.replace(/^https?:\/\//, "")),
                            a = RegExp(a.replace(/([\*\(\)])/g, "\\$1") + "(%26u%3D[^,]*)?,?", "g"),
                            (b = b.replace(a, "").replace(/,$/, "")) ? e.set("Yb_unsent_" + c.id, b) : e.remove("Yb_unsent_" + c.id))
                    },
                    Ia: function() {
                        /**
                         * 执行过程：开始会在 SessionStorage 设置一个 Yb_unsent_id
                         * 这里会最先执行一次，log 成功以后会删掉 Yb_unsent_id
                         * ？
                         */
                        var a = this
                            , b = e.get("Yb_unsent_" + c.id);
                        if (b)
                            for (var b = b.split(","), f = function(b) {
                                d.log(k.O + "//" + decodeURIComponent(b), function(b) {         //  TODO: anchor 1st
                                    a.R(b)
                                })
